...
| **3. Coupling** ✅ | M1 → M2 | Hail = **areal hit-or-miss**: Minkowski `(√F+√s)²/A`, `λ_asset = λ_collection · p`, against the plant footprint. Fixes the old repo's point-factor error (known-answer checked). | `m2_coupling/01_coupling` | [`phase-3-coupling.md`](phase-3-coupling.md); A21; `hazard_math/01`; old `issues/spatial-factor` |
| **4. Damage** ✅ | M2 → M3 | Curated PV `size → damage ratio` curve → per-event **conditional** loss (full loss on hit; `pᵢ` not multiplied in). Scalar v1; distribution/BI deferred. | `m3_damage/01_damage` | [`phase-4-damage.md`](phase-4-damage.md); A22; methodology §6/§12 |
| **5. Loss & metrics** ✅ | M3 → M4 | **Compound-Poisson MC** → annual AEP/OEP → EAL, VaR, PML, TVaR. *The part the old repo broke — done right, with a Method-0 contrast + known-answer checks.* Metrics **real, record-limited** (λ fitted — DD-3 Stage 1). | `m4_loss_metrics/01_loss_metrics` | [`phase-5-loss-metrics.md`](phase-5-loss-metrics.md); A24; methodology §8–§10; risk-metrics ref; `basics_spot_on` |
