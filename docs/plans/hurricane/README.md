...
> **✅ V1 BUILT END-TO-END (M0→M4), hurricane wind × solar.** Everglades Solar (FL, high) + Hayhurst (TX, true-zero).
...
> (JD-TC-1…8). **Next:** swap the damage curve + loss benchmark; then wind-farm V2 (per-turbine field-intensity).
...
> reached later via the reserved `event_family_id`. Decisions: [`decisions.md`](decisions.md) (`JD-TC-1..7`).
...
(shared `layer0`/`m0`/`m1`, then `solar/` for `m2–m4`; `wind_farm/` in V2).
...
1. **Completes the coupling taxonomy** — field-intensity is the third/last bucket (areal · field-intensity ·
   site-conditioned). Built degenerate on solar V1, fully proven at wind-farm V2.
2. **Founds coastal flood (+ compound flooding)** — the shared, storm-resolved **RAFT** catalog + `event_family_id`
   let one storm's wind + surge + rain combine without double-counting. Unlocks flood's deferred coastal `[C]`
...
> EAL/VaR/PML/TVaR (% of TIV + $). **Wind only.** Surge/rain are flood's (hook reserved). Wind farm = V2. Resource
> wind = Performance tier. Field-intensity coupling is **spatially degenerate on solar** (centroid sample) — V1 does
> **not** claim to have proven it; the wind-farm V2 cell does.
...
| **M0** | Meet the evidence: RAFT tracks, IBTrACS/HURDAT2 landfall record, STORM RP grid (cross-check), the two solar sites' geometry & TIV | RAFT NetCDF · IBTrACS (NCEI) · HURDAT2 (NHC) · STORM GeoTIFF (Zenodo) · OSM/EIA geometry | `m0_input_data/01_raft_catalog` · `02_landfall_record_and_rp_crosscheck` · `03_site_geometry` |
| **M1** | **The catalog (the peril, shared):** RAFT tracks → **Holland field** → per-site **storm-resolved** 3-s-gust catalog; frequency from RAFT; **`event_family_id` stamped**; validated vs landfall winds + STORM RP grid | Holland (1980) · gust factor · m/s→mph ×2.237 | `m1_catalog/01_event_catalog` |
...
- **Storm surge / TC rainfall** → flood's `[C]`/`[F]`; reached via `event_family_id` when coastal is built.
- **Wind farm** → hurricane V2 cell (reuses M1 + convective wind's 3-s-gust turbine curve + turbine geometry).
