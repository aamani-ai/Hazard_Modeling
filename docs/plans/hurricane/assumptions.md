...
| ATC-6 | M1 | **Footprint = Holland (1980) field** along-track, **symmetric** (no forward-motion asymmetry — V1 simplification); **Holland B=1.3, RMW** parameterized (sensitivity levers) | [JD-TC-7](decisions.md); Hazard Data Ref §5/§6 | **built** | asymmetry + higher-fidelity field / RAFT gust field |
| ATC-7 | M1 | **Sustained → 3-s gust** via a **gust factor = 1.2** (fixed single value; sensitivity lever, corroborated by ASCE RP-gusts) | [00_hazard_definition](00_hazard_definition.md) | **built** | site/terrain-specific gust factor sourced |
...
| ATC-9 | M1 | **Frequency = OBSERVED HURDAT2 close-passage rate** (distinct ≥64 kt hurricanes within the screen radius ÷ 173 yr); **severity** from the RAFT Holland field — the observed-rate × synthetic-severity split that neutralizes RAFT's ~71× genesis oversample. *(Supersedes the original "RAFT synthetic rate" idea — see [JD-TC-8](decisions.md).)* | [JD-TC-8](decisions.md) | **built** | longer record / per-site rate refinement |
