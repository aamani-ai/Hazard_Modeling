...
## What Hurricane M1 Asks

```text
M1 asks, for each storm and site:
  what Holland wind field is synthesized from the track?
  what peak 3-second gust reaches the site?
  what observed-anchored event rate applies at that site?
  what RAFT storm id becomes event_family_id?
  does the tail match ASCE return-period gusts?
```

It does not ask:

```text
  how much of a solar plant is exposed?
  which turbines see the highest gust?
  what loss does the gust cause?
```

Those are M2 and M3 questions. M1 emits storm-resolved hazard intensity and identity.
