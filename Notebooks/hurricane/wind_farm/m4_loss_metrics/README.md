...
## What Hurricane Wind-Farm M4 Asks

```text
M4 asks, over many simulated years:
  how many hurricane wind events occur?
  which storm loss rows are sampled?
  what annual aggregate wind loss results?
  what is the largest storm loss in the year?
  what EAL, PML, VaR, TVaR, and OEP metrics result?
```

It does not ask how wind and surge combine. This file reports wind-only hurricane loss; flood-coastal M4 performs the
compound surge x wind join on `event_family_id`.
