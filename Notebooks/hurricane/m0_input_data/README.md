...
## What Hurricane M0 Asks

```text
M0 asks:
  what does the RAFT storm catalog contain?
  what units are the track variables in?
  how much does RAFT oversample genesis relative to reality?
  what observed HURDAT2 rate should anchor frequency?
  which sites are the proving, baseline, and flood cross-link riders?
  what site geometry and TIV can later layers use?
```

It does not ask:

```text
  what gust does a storm produce at the site?
