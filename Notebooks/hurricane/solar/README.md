...
## What The Hurricane Solar Cell Asks

```text
solar M2 asks:
  across the solar footprint:
    is the hurricane gust field effectively uniform?
    can one centroid gust represent the plant?
    should value_exposed_fraction be 1?

solar M3 asks:
  for each storm gust:
    what hurricane x solar curve applies?
    what capex-weighted damage ratio results?
    what full conditional storm loss is emitted?

solar M4 asks:
  across simulated years:
    how many storms occur?
    what storm losses are sampled?
