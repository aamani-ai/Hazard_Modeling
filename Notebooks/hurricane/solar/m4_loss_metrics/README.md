...
## What Hurricane Solar M4 Asks

```text
M4 asks, over many simulated years:
  how many hurricane wind events occur?
  which storm event rows are sampled?
  what full conditional wind loss is applied for each storm?
...
```text
  what is EAL?
  what are VaR/PML points?
  what is TVaR99?
  how do AEP and OEP differ when more than one storm can occur?
```
