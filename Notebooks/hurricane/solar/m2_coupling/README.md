...
## What Hurricane Solar M2 Asks

```text
M2 asks, for each storm at the solar site:
  what gust did M1 assign at the centroid?
  how much would gust vary across the footprint?
  is the spread small enough to treat the plant as one point?
  should value_exposed_fraction equal 1?
  what coupled event row should M3 consume?
```
...
```text
  ...
  whether surge also damages the plant?
```

Damage is M3, frequency is M4, and surge is flood.
