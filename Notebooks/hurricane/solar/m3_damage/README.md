...
## What Hurricane Solar M3 Asks

```text
M3 asks, for each storm event:
  what 3-second gust reaches the plant?
  what PV / mounting / substation curves apply?
  what capex-weighted asset damage ratio results?
  what full conditional storm loss is emitted?
  is event identity preserved for downstream joins?
```

It does not ask:

```text
  how many storms happen in a simulated year?
  what is annual PML?
  what surge loss occurs?
```

Those are M4 and flood-compound questions.
