...
```text
M0 asks:
  what storm-resolved synthetic catalog exists?
  what observed HURDAT2 record anchors frequency and validation?
  which solar and wind sites should ride the pipeline?
  what units and oversampling traps must M1 correct?

M1 asks:
  for each storm and site:
    what 3-second gust does the Holland field produce?
    what observed-anchored lambda applies?
    what event_family_id identifies the storm?
    does the return-period tail validate against ASCE?

M2 asks:
  at the asset:
    is the wind field spatially degenerate or not?
    should one centroid gust represent the asset?
    or should every turbine/node be sampled?
    what exposure fraction is carried?

M3 asks:
  given storm gust:
    what solar or turbine damage curve applies?
    what component damage ratios result?
    what full conditional storm loss is emitted?
    is event_family_id preserved for flood compound joins?

M4 asks:
  over many simulated years:
    how many storms occur?
    which storm losses are sampled?
    what annual AEP and OEP vectors result?
    what are EAL, PML, VaR, and TVaR?
```

The core hurricane discipline is:

```text
storms are real event objects
event_family_id is preserved
wind stays in hurricane
surge and rain attach through flood using the same storm identity
```
