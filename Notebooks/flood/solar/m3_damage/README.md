...
The exposure term carries the solar V1 layout assumption:

```text
M2 wet footprint share -> exposed share of the representative solar value mix
```

M3 does not receive separate inverter, transformer, substation, cable, and module coordinates. It assumes the exposed
share contains the same component/capex mix as the whole plant, then applies each component curve at the conditional
wet depth:

```text
Asset_DR(depth)
  = inverter_weight   * DR_inverter(depth)
  + substation_weight * DR_substation(depth)
  + pv_weight         * DR_pv(depth)
  + civil_weight      * DR_civil(depth)
  + electrical_weight * DR_electrical(depth)

conditional_loss
  = exposure_fraction * Asset_DR(conditional_depth) * TIV
```

This is why the model can price solar flood without a sub-asset layout, and also why the result is sensitive to where
low electrical equipment actually sits.
...
## Method In Plain English: Depth Becomes Solar Damage

M3 takes the M2 coupling row and asks: "If this exposed part of the solar plant sees this much water, what fraction of
value is damaged?"
...
M2 row
  ...
  conditional_depth
        ...
        v
M3 curve
  depth -> subsystem damage ratios
        |
        v
capex weighting
  subsystem DRs -> asset damage ratio
        |
        v
conditional loss
  exposure * asset_DR * TIV
```

The flood x solar curve is really a stack of component curves:

```text
damage ratio
  ^
1 |                         PV / substation caps
  |                    ____/
  |              _____/
  |        _____/
  |  _____/  inverter starts early
0 +-------------------------------------------------> depth above ground
       0 ft      0.75 ft       2 ft        deeper
```
...
```text
shallow water:
  may reach inverter pads and low electrical equipment

moderate water:
  reaches substations, civil/electrical systems

deep water:
  more subsystems saturate, but damage ratios cap below or near total loss
```

Worked shape:

```text
conditional_depth = 0.433 m = 1.42 ft
exposure_fraction = 0.177
Asset_DR(1.42 ft) = read from capex-weighted solar flood curve

conditional_loss = 0.177 * Asset_DR(1.42 ft) * TIV
```

M3 uses the same depth-damage curve for riverine, pluvial, and coastal surge because the damage driver is water depth
above ground. The source of the water matters in M1/M2/M4; the equipment response in M3 is source-agnostic in this V1.

## What Solar M3 Asks

```text
solar M3 asks, for each M2 row:
  what conditional depth reaches the wet part of the plant?
  which components start taking damage at that depth?
  what damage ratio applies to each component?
  how do component capex weights combine into Asset_DR?
  what conditional loss equals exposure * Asset_DR * TIV?
```

It does not ask:

```text
  what is the return period frequency?
  how should riverine and pluvial combine?
  what is PML100?
```

Those are M1/M4 questions.
