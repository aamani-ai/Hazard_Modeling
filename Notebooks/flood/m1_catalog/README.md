...
M1 converts source evidence into a hazard object that is still independent of the asset shape. The asset-specific step
comes later in M2.

```text
M1 does not ask:
  "How much of this solar farm floods?"
  "Which wind turbines flood?"
  "What is the dollar loss?"

M1 asks:
  "What flood field or storm catalog exists at this site?"
```
...
```text
Riverine [R]
  return period T -> flood depth field

Pluvial [F]
  return period T -> runoff depth source term

Coastal [C]
  storm event_family_id -> category -> surge field
```

The inland branches are annual-maximum curves:

```text
T = 10, 25, 50, 100, 500 years
AEP = 1/T

larger T  -> rarer flood
larger T  -> usually larger depth/loss
```

The coastal branch is event-based because it has to join hurricane wind on the same storm:

```text
storm A:
  event_family_id = A
  hurricane wind loss row exists
  coastal surge loss row exists
  M4 combines them as the same occurrence
```

This is the core reason M1 forks. The physics are all "water depth causes damage", but the evidence format is different
enough that forcing all three sub-perils into one catalog shape would hide important assumptions.

## What M1 Asks

```text
M1 asks:
  for each site and sub-peril:
    what is the hazard source?
    what is the frequency basis?
    what is the magnitude variable?
    is the output a return-period field or a storm event catalog?
    what bridge assumptions are needed before M2 can sample the asset?
```
...
```text
riverine asks:
  do we have BLE depth rasters, a Zone-A bathtub fallback, or dry?
  what Q(T) curve supports missing return periods?

pluvial asks:
  what Atlas 14 rainfall frequency exists?
  what SCS-CN runoff Q(T) does that imply?
  what is deferred to terrain ponding in M2?

coastal asks:
  which storms pass close enough and strong enough?
  what annual rate lambda is observed-anchored?
  what event_family_id will join surge to hurricane wind?
```
