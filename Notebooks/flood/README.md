...
## Reading The Flood Notebooks Without Opening The Code

The flood notebooks have one job: convert public flood evidence into the same loss-distribution contract used by the
other hazards. The confusing part is that flood is not one data shape. Riverine and pluvial are return-period curves;
coastal is a per-storm event catalog. The notebooks keep those shapes separate until M4.

```text
layer-0
  Define the flood family:
    riverine [R]       pluvial [F]          coastal [C]
    return period      return period        storm event
    depth field        runoff source        surge by category
          \                 |                    /
           \                |                   /
            v               v                  v
M0  raw public evidence and site geometry
    NFHL / BLE / DEM / Atlas 14 / SLOSH / tracks / asset footprints

M1  asset-independent hazard field or event catalog
    R: T -> flood depth field
    F: T -> runoff depth source term
    C: storm_id -> category -> surge field

M2  asset coupling
    solar: polygon fraction wet + conditional depth
    wind:  per-node turbine/substation depth

M3  damage
    depth above ground -> damage ratio -> conditional loss

M4  annual loss distribution
    inland:  annual-maximum RP sampling, max(riverine, pluvial)
    coastal: compound-Poisson storm sampling, joined to hurricane wind
    total:   inland + coastal
```

The key discipline is that every layer emits a specific object, not a vague "risk score":

```text
M0 output: source evidence + site geometry
M1 output: hazard field / event catalog
M2 output: exposure and depth at the asset
M3 output: conditional loss by return period or event
M4 output: sampled annual loss distribution and metrics
```

For the inland branch, a row can be read like this:

```text
Return period T
  -> annual exceedance probability AEP = 1 / T
  -> flood source magnitude at that T
  -> asset exposure_fraction and conditional_depth
  -> conditional_loss_T
```

For the coastal branch, a row can be read like this:

```text
Storm event_family_id
  -> hurricane category
  -> SLOSH surge depth at the site
  -> asset exposure/depth
  -> surge loss for that same storm
  -> M4 joins hurricane wind loss on event_family_id
```

That is why flood has more README structure than hail or wildfire: the same final contract is reached through two
different input modes.

## M0-M4: What Each Layer Asks

Use this as the fast recap when returning to the folder after time away.

```text
layer-0 asks:
  what flood family are we modeling?
  what are the sub-perils?
  what is the shared damage driver?
  where does frequency live?
  where does equipment damage begin?

M0 asks:
  which asset sites should be modeled?
  what geometry do we trust?
  what public flood sources touch the site?
  is a zero a true dry reading or missing data?

M1 asks:
  for each sub-peril, what hazard field or event catalog exists?
  what is the frequency basis?
  what source variable is being carried forward?
  what assumptions bridge missing depths or lower return periods?

M2 asks:
  at the actual asset:
    which pixels or nodes are wet?
    how many are wet?
    what is the average depth where wet?
    what is the maximum local depth?

M3 asks:
  given that water depth:
    which components are reached?
    what damage ratio applies?
    how do component weights combine to asset loss?

M4 asks:
  over many simulated years:
    what inland annual-max loss occurs?
    what coastal storm losses occur?
    what is the annual aggregate loss?
    what are EAL, PML, VaR, TVaR, and OEP?
```

There is no built M5 layer in this notebook contract today. If we add one later, it should probably be the platform
integration layer: "how do these asset loss distributions feed portfolio aggregation, pricing, reporting, and decision
workflows?"
