...
2. **Wind flood is capped at the turbines; the collector is a scope-sensitive dependency node** — rotor/nacelle/tower
   (0.63 of value) are flood-immune (a fully-inundated turbine loses only ~28%), so the **turbines-only floor is small
   (provisional EAL 0.31% at Green River)**. The mapped collector node sits in the valley and floods, so it is
   load-bearing for dependency/disruption screening and for any owner-confirmed collector sensitivity. It should not be
   treated as baseline physical damage unless ownership and TIV inclusion are confirmed.
...
This folder is the wind-farm version of the flood asset cell. The shared flood catalog is the same, but the coupling is
different because a wind farm is sparse: turbines and substations are points, not a dense value-filled polygon.

```text
shared flood catalog
  R: return period -> riverine field / bathtub
  F: return period -> pluvial runoff
  C: storm -> coastal surge field
        |
        v
wind M2
  each turbine/substation reads its own depth
        |
        v
wind M3
  per-node depth -> component damage -> node/substation loss
        |
        v
wind M4
...
The key contrast with solar:

```text
solar:
  many raster cells inside one footprint
  exposure_fraction = wet area / total area

wind:
  many discrete nodes
  flooded_fraction = wet turbines / total turbines
  plus a separate collector substation node
```

The collector substation is not supplied by USWTDB. The notebook finds it with a turbine-cloud hull:

```text
USWTDB turbine points

        T
   T         T

      T       S?       S? = candidate OSM/HIFLD substation
...
```text
   ...
   T /     \ T
    /       \
   |    T    |      candidate inside / near hull can be this farm's collector
   |   S?    |
 T |         | T
    \       /
     \  T  /
...
```text
prefer OSM substation=generation inside the hull
allow OSM generation substation just outside if nearest turbine < 0.6 km
otherwise use in-hull OSM/HIFLD
centroid only as flagged fallback
```

This is a positive part of the wind-flood exposure method. It prevents the bad shortcut "nearest substation to
centroid," which previously grabbed a neighboring farm's dry collector at Green River. It is not, by itself, proof that
the candidate collector belongs in the wind farm's physical-damage TIV. Baseline physical loss should include the
collector only when ownership / value-schedule evidence confirms it; otherwise this node is a dependency or disruption
screen.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ water
     T1       T2       T3       SUB
     |        |        |        |
   pad      pad      pad      lower/electrical

effective node depth = water surface - ground - pad height
```

This is why the collector substation is load-bearing in the wind results. A few turbines in shallow water may not drive
large loss, but a valley-bottom collector substation can dominate the farm-level damage.

## What The Wind Asset Cell Asks

```text
wind M2 asks:
  for each turbine and collector substation:
    is water above ground?
    is water above the pad?
    what is the effective depth at that node?
    how many turbines are flooded?

wind M3 asks:
  for each flooded node:
    which base components are vulnerable?
    which high components are flood-immune?
    what node loss does that create?
    how much does the collector substation contribute?

wind M4 asks:
  across simulated years:
    what inland annual-max node-summed loss occurs?
    what coastal surge plus hurricane wind loss occurs?
    how different is the full headline from the turbines-only floor?
```
