...
## Method In Plain English: Wind Damage Is Base-Heavy

M3 turns each flooded node depth into loss. The wind-specific point is that most turbine value is high above the water.

```text
wind turbine value by height

        rotor / nacelle          flood-immune in V1
              |
              |
              |
             tower               flood-immune in V1
              |
--------------+---------------- ground / pad
 foundation + down-tower electrical + civil
 vulnerable only near the base
```

So the damage cap for a fully flooded turbine is much lower than a fully flooded solar plant:

```text
turbine value:
  rotor + nacelle + tower  -> DR = 0 for flood
  foundation/electrical/civil -> depth-damage curves

collector substation:
  separate node with steep electrical damage curve
```

Per-node calculation:

```text
for each flooded turbine i:
  turbine_loss_i =
    foundation_weight * DR_foundation(depth_i) +
    electrical_weight * DR_electrical(depth_i) +
    civil_weight      * DR_civil(depth_i)

substation_loss =
  substation_weight * DR_substation(depth_sub)

farm_loss = sum(turbine_loss_i over nodes) + substation_loss
```

ASCII curve intuition:

```text
damage ratio
  ^
  |             substation / electrical rises early
  |          __/
  |      ___/
  |  ___/        turbine total remains capped
  +-------------------------------------------------> water depth
       shallow        moderate          deep
```

This is why the Green River result has a turbines-only floor and a full headline with the collector included. The
substation location is not a detail; it is part of the exposure model.

## What Wind M3 Asks

```text
wind M3 asks, for each flooded turbine:
  does water reach base electrical equipment?
  does water reach civil/foundation components?
  which high-value components remain flood-immune?
  what capped turbine damage ratio results?

for the collector substation:
  what depth reaches low electrical equipment?
  what substation damage ratio applies?

for the farm:
  what node-summed conditional loss is emitted to M4?
```

It does not ask whether this is the annual maximum or one storm among many. That timing/frequency logic belongs in M4.
