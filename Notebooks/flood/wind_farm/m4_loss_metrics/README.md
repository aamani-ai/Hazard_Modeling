...
**Collector bracket (JD-FL-W7):** the provisional notebook reports a **turbines-only floor** and a collector-included
sensitivity. The mapped west-edge 138 kV collector/dependency node sits in the valley and floods, so it is load-bearing
for exposure and disruption screening. Treat the collector-included result as physical damage only if ownership and
TIV/insured-value inclusion are confirmed.
...
- **Green River is riverine-dominated in the provisional run** — the mapped valley-bottom collector/dependency node
  floods (0.88–1.00 m from the 10-yr on), producing a large collector-included sensitivity. The turbines-only floor
  (collector excluded) is EAL ~0.31% / PML500 ~3.2%. The collector-included physical-loss headline is scope-flagged
  pending ownership/TIV confirmation.
...
## Method In Plain English: Node Losses Become Farm Annual Metrics

M4 uses the same annual-distribution idea as solar, but the input losses came from node-summed turbine and substation
damage rather than polygon exposure.
...
one simulated year
        |
        v
draw annual flood severity u
        |
        v
read riverine farm loss at u
read pluvial  farm loss at u
        |
        v
inland farm loss = max(riverine, pluvial)
```

The `max` rule is the same flood principle as solar: one annual inland flood state should not double-damage the same
equipment simply because two source models exist.
...
one simulated year
        |
        v
draw number of coastal storms ~ Poisson(lambda)
        |
        v
for each storm:
  surge node losses from flood x wind
  hurricane wind node losses from hurricane x wind
  join on event_family_id
...
```text
annual loss vector, length 500k
        |
        +--> EAL     = average
        +--> VaR99   = 99th percentile
        +--> PML100  = 100-year annual-exceedance point
        +--> PML500  = 500-year annual-exceedance point
        +--> TVaR99  = mean beyond the 99th percentile
        +--> OEP     = largest occurrence per year
```

The wind-specific sensitivity is the substation bracket:

```text
turbines-only floor:
  excludes collector substation

full headline:
  includes collector substation

if these differ a lot:
  the collector location is driving flood risk
```

That is exactly what happens at Green River: the turbines themselves are capped, but the collector sits in the flood
valley and carries most of the modeled EAL.

## What Wind M4 Asks

```text
wind M4 asks, over many simulated years:
  what inland flood severity is drawn?
  what riverine farm loss does that imply?
  what pluvial farm loss does that imply?
  after worse-source-wins, what is inland loss?
  how many coastal storms occur?
  for each coastal storm, what surge loss and hurricane wind loss share event_family_id?
  what is the total farm annual loss?
```

Then it asks the wind-specific sensitivity question:

```text
  what is the turbines-only loss floor?
  what is the full loss with the collector substation?
  is the collector location driving the headline?
```
